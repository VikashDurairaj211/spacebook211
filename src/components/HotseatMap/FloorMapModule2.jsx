...
function Section({
  ...
  return (
    <section className="m2-section">
      <div className="m2-section-title">{title}</div>

      <div
        className="m2-column-labels"
        style={{
          gridTemplateColumns: `var(--m2-label) repeat(${columns}, var(--m2-seat))`,
        }}
      >
        <span />
        ...
        {rows.map((row, rowIndex) => (
          ...
          >
            ...
            {row.map((number, columnIndex) => {
              ...
              return (
                <div className="m2-seat-cell" key={number}>
                  {seat && (
                    <Seat
                      seat={seat}
...
function Room({ children, className = "" }) {
  return (
    <div className={`m2-room ${className}`}>
      {children}
    </div>
  );
}
